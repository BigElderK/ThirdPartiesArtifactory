#!/bin/bash
DEV_ENV_CONAN_ROOT_PATH=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
...
conan install $DEV_ENV_CONAN_ROOT_PATH/conanfile.txt --update --generator CMakeToolchain --output-folder $DEV_ENV_CONAN_ROOT_PATH/_generated/host/macos --build never --profile=$DEV_ENV_CONAN_ROOT_PATH/profiles/host/conan_host_profile.macos.txt
