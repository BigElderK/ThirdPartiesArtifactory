@echo off
set BUILD_ENV_CONAN_ROOT_PATH=%~dp0
...
conan install %BUILD_ENV_CONAN_ROOT_PATH%\conanfile.txt --update --generator CMakeToolchain --output-folder %BUILD_ENV_CONAN_ROOT_PATH%\_generated\host\windows --build never --profile=%BUILD_ENV_CONAN_ROOT_PATH%\profiles\host\conan_host_profile.windows.txt
